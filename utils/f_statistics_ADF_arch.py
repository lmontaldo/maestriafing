...
class TestStatistics:
    @staticmethod
    def tau_tau(df, columns=None, critical_value=-3.41):
        """
        ...
        - columns: list of str or None, optional
            A list of column names to include in the unit root test. If None, all columns are included (default: None).
    ...
    @staticmethod
    def phi_2(df, critical_value=4.68, r=3, alpha=0.05, cols_norho=None):
        """
    ...
    def phi_3(df, critical_value=7.44, r=2, alpha=0.05, cols_norho=None):
        ...
        - alpha: float, optional
            The significance level (default: 0.05).
        - cols_norho: list of str or None, optional
            A list of column names to include in the test. If None, all columns are included (default: None).
    ...
    @staticmethod
    def test_gamma_is_zero_a2(series, significance_level=0.05):
        adf_test = ADF(series, trend='ct')
        gamma_hat = adf_test.gamma
        se_gamma = adf_test.std_errors[0]
        
        t_stat = gamma_hat / se_gamma
        df = len(series) - 1
        critical_value = t.ppf(1 - significance_level/2, df)
        return t_stat, critical_value
    def test_gamma_normal_distribution_a2(df, cols_rho=None, significance_level=0.05):
        rho_list = []  # Columns where we reject the null hypothesis
        no_rho_list = []  # Columns where we fail to reject the null hypothesis
        ...
        for column in df_sliced.columns:
            t_stat, critical_value = TestStatistics.test_gamma_is_zero_a2(df_sliced[column], significance_level)
            
            if abs(t_stat) > critical_value:
                rho_list.append(column)
            else:
                no_rho_list.append(column)
        
        rh0 = df_sliced[rho_list]
        no_rh0 = df_sliced[no_rho_list]
        
        results = {
            'rh0': rh0,
            'no_rh0': no_rh0,
            'rho_list': rho_list,
            'no_rho_list': no_rho_list
        }
        
        return results 
    
    @staticmethod
    def tau_mu(df, critical_value=-2.90, return_values=None, columns=None):
        """
        ...
        - return_values: list of str, optional
            A list of values to return (default: None).
        - columns: list of str, optional
            A list of column names to process (default: None, processes all columns).
    ...
    @staticmethod
    def test_gamma_is_zero_a0(series, significance_level=0.05):
        adf_test = ADF(series, trend='c')
        gamma_hat = adf_test.gamma
        se_gamma = adf_test.std_errors[0]
    ...
    def test_gamma_normal_distribution_a0(df, cols_rho=None, significance_level=0.05):
        rho_list = []  # Columns where we reject the null hypothesis
        no_rho_list = []  # Columns where we fail to reject the null hypothesis
        ...
        for column in df_sliced.columns:
            t_stat, critical_value = TestStatistics.test_gamma_is_zero_a0(df_sliced[column], significance_level)
